...
For **CLI skill execution** (`free_agent` route), the agentic execution SHALL use the two-layer milestone architecture: a Milestone Planner first decomposes the command into coarse milestones, and a Milestone Executor runs a focused Think–Act–Observe sub-loop per milestone. The single-level `_dynamic_execute_loop()` SHALL no longer be the primary execution path for `free_agent` commands.
...
#### Scenario: CLI free_agent command uses milestone architecture
- **WHEN** the CLI routes a command to `free_agent`
- **THEN** `plan_milestones()` is called first, producing an ordered `Milestone` list
- **AND** `_milestone_execute_loop()` iterates over milestones, running a sub-loop per milestone
- **AND** the flat `_dynamic_execute_loop()` is NOT called for this command
