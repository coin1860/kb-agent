...
- **AND** Chunk 设置大小需要控制在阈值（如 `max_chars` 限制，默认 800 字符）以下

#### Scenario: Long paragraphs split with overlap
- **WHEN** 分析到超过 `max_chars` 长度的连续大段正文（无 Header 分割）
- **THEN** 系统将其通过换行符或段落切为不同 Chunks
- **AND** 系统在临近块的交界处需带上适量的 Overlap (`overlap_chars`)
...
The system SHALL use optimized chunk sizes for document parsing, specifically `max_chars=800` and `overlap_chars=200` by default, to improve relevance density. These values SHALL be configurable via `Settings`.

#### Scenario: Markdown processing with custom sizes
- **WHEN** a `.md` file is processed by `MarkdownAwareChunker`
- **THEN** chunks are split at the configured `max_chars` or the nearest paragraph boundary
- **AND** adjacent chunks have the configured `overlap_chars` overlap
